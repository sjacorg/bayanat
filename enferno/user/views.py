...
def auth() -> Response:
    ...
    if not cfg.GOOGLE_OAUTH_ENABLED or not cfg.GOOGLE_CLIENT_ALLOWED_DOMAIN:
        return HTTPResponse.json_error(
            "Google Auth is not enabled or configured properly", status=417
        )
...
def auth_callback() -> Response:
    ...
    if userinfo_response.json().get("email_verified"):
        ...
        return HTTPResponse.json_error(
            "User email not available or not verified by Google.", status=400
        )

    # check if email belongs to the allowed domain
    if not users_email.split("@")[-1] == cfg.GOOGLE_CLIENT_ALLOWED_DOMAIN:
        return HTTPResponse.json_error("User email rejected!", status=403)
    ...
    if u is None:
        ...
        return HTTPResponse.json_error(
            "User not found. Ask an administrator to create an account for you.", status=404
        )
...
def save_settings() -> Response:
    ...
    if not user:
        return HTTPResponse.json_error("Problem loading user", status=417)
    ...
    user.save()
    return HTTPResponse.json_ok(message="Settings Saved", status=200)
...
def load_settings() -> Response:
    ...
    return HTTPResponse.json_ok(settings, status=200)
