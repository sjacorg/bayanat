    ...
    template: `
        ...
        <v-card color="grey lighten-3" class="mx-auto pa-3">
            ...
            <v-card-text>
                ...
                <v-chip :href="bulletin.source_link" target="_blank" small pill label color="lime darken-3 " class="white--text ml-1">
                    # {{bulletin.originid}}</v-chip>
                <v-btn v-if="editAllowed()" class="ml-2" @click="$emit('edit',bulletin)" x-small outlined>Edit</v-btn>

            </v-card-text>
            
                <v-sheet   v-if="bulletin.ref && bulletin.ref.length" outlined class="ma-2 pa-2 d-flex align-center flex-grow-1" color="yellow lighten-5 " > 
            ...
            <v-card v-if="bulletin.review" outline elevation="0" class="ma-3" color="light-green lighten-5">
                <v-card-text>
