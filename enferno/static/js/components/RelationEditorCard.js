const RelationEditorCard = Vue.defineComponent({
  ...
  template: `
    <v-card>
      <v-card-item>
        <v-card-title>{{ title || translations.relation_ }} <v-chip v-if="$root.relationToConfirm?.id" prepend-icon="mdi-identifier" class="ml-1">{{ $root.relationToConfirm?.id }}</v-chip></v-card-title>
      </v-card-item>
      <v-card-text>
...
window.RelationEditorCard = RelationEditorCard;
