    ...
    formatTextDiffString(context, value) {
        ...
        for (let i = 0, l = lines.length; i < l; i++) {
            ...
            for (const piece of pieces) {
                context.out(`<span class="jsondiffpatch-textdiff-${piece.type}">${htmlEscape(piece.text)}</span>`);
            }
            context.out("</div></li>");
        }
        context.out("</ul>");
...
const wrapPropertyName = (name) => `<pre style="display:inline-block">&quot;${htmlEscape(name !== null && name !== void 0 ? name : "")}&quot;</pre>`;
